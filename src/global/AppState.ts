...
class AppState {
  ...
  getNodeOfId(id: number) {
    return getNonLeafNodeByPath(this.notebooks, this.id2NotePathMap.get(id))
  }

  @action focusChapter(chapter: Chapter) {
    this.focusedPath = this.id2NotePathMap.get(chapter.id)
    this.focusNoteList(chapter.notes)
  }

  @action focusNoteList(notes: number[]) {
